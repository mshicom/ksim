...
class DistributedRLTask(RLTask[Config], Generic[Config], ABC):
    ...
    def __init__(self, config: Config) -> None:
        ...
        logger.info(
            f"Distributed training setup: {self.local_device_count} local devices, "
    ...
    def verify_distributed_setup(self) -> None:
        ...
        logger.info(f"Total devices: {self.local_device_count}")
        logger.info(f"Total environments: {self.config.num_envs}")
        logger.info(f"Environments per device: {self.num_envs_per_device}")
        logger.info(f"Batch size: {self.config.batch_size}")
        logger.info(f"Number of batches: {self.num_batches}")
        logger.info(f"Pmap axis name: {self.config.pmap_axis_name}")
        logger.info(f"Gradient sync period: {self.config.grad_sync_period}")
        logger.info("==================================")
